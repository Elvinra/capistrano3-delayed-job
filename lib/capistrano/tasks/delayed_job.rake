...
namespace :delayed_job do
    ...
    on roles(delayed_job_roles) do
      within release_path do
        with rails_env: fetch(:rails_env) do
          execute :bundle, fetch(:bundle_version), :exec, delayed_job_bin, delayed_job_args, :stop
        end
      end
    end
  end
  ...
  task :start do
    on roles(delayed_job_roles) do
      within release_path do
        with rails_env: fetch(:rails_env) do
          execute :bundle, fetch(:bundle_version), :exec, delayed_job_bin, delayed_job_args, :start
        end
      end
    end
  end
  ...
  task :status do
    on roles(delayed_job_roles) do
      within release_path do
        with rails_env: fetch(:rails_env) do
          capture( :bundle, fetch(:bundle_version), :exec, delayed_job_bin, delayed_job_args, :status ).each_line do |line|
            info line
          end
        end
      end
  ...
  task :restart do
    if fetch(:delayed_job_default_hooks)
      on roles(delayed_job_roles) do
        within release_path do
          with rails_env: fetch(:rails_env) do
            execute :bundle, fetch(:bundle_version), :exec, delayed_job_bin, delayed_job_args, :restart
          end
        end
      end
    end
